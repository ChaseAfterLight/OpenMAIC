...
export interface GenerateClassroomInput {
  ...
  stageSeed?: Pick<
    Stage,
    'id' | 'name' | 'description' | 'language' | 'languageDirective' | 'style' | 'lessonPack'
  >;
...
export async function generateClassroom(
  ...
  const outlinePayload = Array.isArray(outlinesResult.data)
    ? {
        languageDirective:
          input.language === 'en-US'
            ? 'Teach in English (en-US).'
            : 'Teach in Chinese (Simplified, zh-CN).',
        outlines: outlinesResult.data,
      }
    : outlinesResult.data;
  const { languageDirective, outlines } = outlinePayload;
